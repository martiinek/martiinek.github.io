...
export const Navbar: React.FC<NavbarProps> = ({ scrolled }) => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-black/60 backdrop-blur-xl border-b border-gray-800 py-4' : 'bg-transparent py-10'}`}>
      <div className="max-w-7xl mx-auto px-6 lg:px-32 flex justify-end items-center">
        <div className="flex items-center space-x-8 md:space-x-12">
          {links.map(link => (
            ...
            >
              <span className="relative z-14">{link.name}</span>
